<#
.SYNOPSIS
    OOrum Uravum — Full Build & Deploy Package Script (Windows)
...
function Fix-WindowsPaths($file, $projectSubPath) {
    ...
    # Replace full Windows project path variants
    $winPath1 = $ROOT.Replace('\', '\\')
    $winPath2 = $ROOT.Replace('\', '/')
    $winPath3 = $ROOT
    $content = $content -replace [regex]::Escape($winPath1 + '\\' + $projectSubPath), ''
    $content = $content -replace [regex]::Escape($winPath2 + '/' + $projectSubPath), ''
    $content = $content -replace [regex]::Escape($winPath3 + '\' + $projectSubPath), ''
    $content = $content -replace [regex]::Escape($winPath3 + '/' + $projectSubPath), ''
    # Also replace any remaining C:\... absolute paths in the file
    $content = $content -replace 'C:\\\\[^"'']+\\\\' + $projectSubPath.Replace('\','\\\\') + '\\\\?', ''
    $content = $content -replace 'C:/[^"'']+/' + $projectSubPath.Replace('\','/') + '/?', ''
...
Write-Host "  OOrum Uravum — Build & Deploy Packager" -ForegroundColor Magenta
...
if ($buildAll) {
    if (Test-Path $DEPLOY) {
        ...
        Write-Success "Removed old deploy/"
    }
...
if ($doBackend) {
    ...
    Copy-Item (Join-Path $BE_SRC "package-lock.json") $BE_DEST -ErrorAction SilentlyContinue

    # Copy prisma schema + migrations (needed for migrate deploy on server)
    $prismaDir = Join-Path $ROOT "backend" | Join-Path -ChildPath "prisma" -ErrorAction SilentlyContinue
    # Try backend/prisma first, then root-level prisma
    $prismaSrc = $null
    ...
    if ($prismaSrc) {
        ...
        Write-Success "Prisma schema copied from $prismaSrc"
    } else {
        Write-Warn "prisma/ folder not found — skipping"
    }

    # Copy Prisma generated client (needed on server without generate)
    ...
    if (Test-Path $prismaClient) {
        ...
        Write-Success "Prisma .prisma client copied"
    }
    ...
    if (Test-Path $envSrc) {
        ...
        Write-Success ".env.production → .env"
    } else {
        Write-Warn ".env.production not found — you must create deploy/backend/.env manually"
    }

    # Create uploads directory placeholder
    New-Item -ItemType Directory -Path (Join-Path $BE_DEST "uploads") -Force | Out-Null

    Write-Success "Backend package ready at deploy/backend/"
    ...
    if (-not $SkipZip) {
        ...
        Write-Success "deploy/backend.zip created"
    }
}
...
if ($doCms) {
    ...
    Write-Success "Next.js build done"
    ...
    if (-not (Test-Path $standaloneSrc)) {
        # Some Next.js versions output differently
    ...
    if (Test-Path $staticSrc) {
        ...
        Write-Success ".next/static copied"
    }
    ...
    $rsfPath = Join-Path $CMS_DEST ".next" "required-server-files.json"
    Fix-WindowsPaths $rsfPath "publisher-dashboard"
    Write-Success "Windows paths fixed"

    # Rename node_modules → _modules (cPanel Passenger wrapper uses this)
    $nmSrc = Join-Path $CMS_DEST "node_modules"
    $mmDest = Join-Path $CMS_DEST "_modules"
    if (Test-Path $nmSrc) {
        Rename-Item $nmSrc "_modules"
        Write-Success "node_modules → _modules"
    }

    # Write start.js wrapper (sets NODE_PATH to _modules for Passenger)
    $startJs = @'
const p=require('path'),M=require('module'),_r=M._resolveFilename.bind(M);
M._resolveFilename=function(r,...a){
  if(!r.startsWith('.')&&!r.startsWith('/')&&!r.startsWith('node:')){
    try{return _r(p.join(__dirname,'_modules',r),...a)}catch(e){}
  }
  return _r(r,...a)
};
process.env.NODE_PATH=p.join(__dirname,'_modules');
M._initPaths();
require('./server.js');
'@
    Set-Content (Join-Path $CMS_DEST "start.js") $startJs -Encoding UTF8
    Write-Success "start.js created"

    # Write .htaccess for Passenger
    $htaccess = @'
...
'@
    ...
    if (-not $SkipZip) {
        ...
        Write-Success "deploy/cms.zip created"
    }
}
...
if ($doFrontend) {
    ...
    $rsfPath = Join-Path $FE_DEST ".next" "required-server-files.json"
    Fix-WindowsPaths $rsfPath "frontend"
    Write-Success "Windows paths fixed"

    # IMPORTANT: Remove app.js if it exists — Passenger loads it instead of server.js
    $appJs = Join-Path $FE_DEST "app.js"
    if (Test-Path $appJs) {
        Remove-Item $appJs -Force
        Write-Warn "Removed app.js (would cause Passenger to load wrong file)"
    }
...
'@
    ...
    Write-Success "Frontend package ready at deploy/frontend/"
    ...
    if (-not $SkipZip) {
        ...
        Write-Success "deploy/frontend.zip created"
    }
}
...
Write-Host "============================================" -ForegroundColor Magenta
if (-not $SkipZip) {
    Write-Host "`n  Upload these files to cPanel:" -ForegroundColor Cyan
    ...
    Write-Host "`n  Then SSH in and run the server-side setup (see DEPLOYMENT.md Step 4)" -ForegroundColor Cyan
}
Write-Host ""
